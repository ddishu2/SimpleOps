...
require_once __DIR__.DIRECTORY_SEPARATOR.'Proposals.php';
         ...
         $app->
            get(cl_RMGTool_Globals ::GC_ROUTE_DEPLOYABLE_EMPS, 
               function () use ($app)
               {
                    ...
                    $lv_so_project_name  = $app->request->get(cl_vo_open_sos::C_PROJECT_NAME);$lv_so_project_id  = $app->request->get(cl_vo_open_sos::C_PROJECT_ID);
                    $lv_so_cust_name     = $app->request->get(cl_vo_open_sos::C_CUST_NAME);
                    $lv_so_capability    = $app->request->get(cl_vo_open_sos::C_CAPABILITY);
                    $lo_open_sos = new open_sos($lv_so_from_date, $lv_so_to_date);
               
//                    $lt_open_sos = $lo_open_sos->get($lv_so_from_date, $lv_so_to_date);
                  $lo_deployable_emp = new cl_deployableBUEmps();  
                    
                    $c_pg = new Proposals($lo_open_sos,$lo_deployable_emp,$lv_so_project_name,$lv_so_project_bu,$lv_so_location,$lv_so_project_id,$lv_so_capability,$lv_so_cust_name);
