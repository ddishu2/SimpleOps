...
//                    $app->response->headers->set('Content-Type', 'application/json');
                    ...
                    $lo_queryBuild = new cl_vo_open_sos('2015-09-28', '2015-09-30');
                    $lo_queryBuild->filterByContainsProjectName($lv_so_projname);
                    $lo_queryBuild->filterByEqualsProjBU($lv_so_proj_bu);
                    $lo_queryBuild->filterByInLocationList($larr_so_locs);
                    $larr_open_so = $lo_queryBuild->get();
                    $app->response->setStatus(200);
                    $app->response->headers->set('Content-Type', 'application/json');
